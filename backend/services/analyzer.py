...
def run_analysis(code: str, features: dict, mode: str = "local", api_key: Optional[str] = None, api_key_source: Optional[str] = None):
    ...
    try:
        docs_links = []
        # simple static mapping for common libraries / packages
        DOC_URLS = {
        ...
        # if docs were produced as list of dependency strings, try to extract names
        for d in raw_docs:
            try:
                # d might be 'name — reason' as generated by validate_docs; split on '—' or '-' heuristically
                name = None
                if isinstance(d, dict):
                    ...
                    if '—' in d:
                        ...
                        name = str(d).split()[0].strip().strip('[],()')
                if not name:
                    continue
                lower = name.lower()
                if lower in DOC_URLS:
                    docs_links.append({'name': name, 'url': DOC_URLS[lower]})
            except Exception:
                continue
        if docs_links:
